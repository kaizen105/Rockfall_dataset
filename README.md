...
**Dataset Size**: 500 Locations, 2,436 Events
...
- `Profile_Clipped` (Boolean): Flag indicating if the simulation prematurely exited due to reaching the bounds of the extracted DEM profile before coming to a natural rest.
...
- `State`: Kinematic state of the object (`BOUNCING` encompasses both freefall and bouncing physics, `SLIDING` for sliding friction).
...
The 1D slope stability model computes Factor of Safety (FS): `FS = [ c + (γ · z · cos²β - u) · tanφ ] / [ γ · z · sinβ · cosβ ]`
...
- **Impact Velocity Decomposition**: Velocity vectors are decomposed against the local ground slope tangent ($\vec{t}$) and normal ($\vec{n}$):
  $v_{in,n} = \vec{v}_{in} \cdot \vec{n}$
  $v_{in,t} = \vec{v}_{in} \cdot \vec{t}$
- **Restitution (CRSP Model)**: Uses Colorado Rockfall Simulation Program (CRSP) industry-standard parameters for vegetated talus slopes ($R_n = 0.35$, $R_t = 0.85$, $\mu = 0.15$).
  $v_{out,n} = -R_n \cdot v_{in,n}$
  $v_{out,t} = R_t \cdot v_{in,t}$
- **Sliding / Rolling Friction**: Triggered when normal velocity drops below a bounce threshold ($v_{out,n} < 0.5$ m/s).
...
- **Truncation Check**: The vast majority of boulders naturally halted due to simulated physical friction rather than array boundaries. Trajectories clipped by DEM boundaries have been marked via `Profile_Clipped`.
